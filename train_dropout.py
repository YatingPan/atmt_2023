...
def get_args():
    ...
    # Add word dropout rate argument
    parser.add_argument('--word-dropout-rate', type=float, default=0.1,
                        help='Probability of dropping a word in the source sentence during training.')
...
def word_dropout(tokens, dropout_rate, pad_idx):
    """Drop words from a sentence with a given probability."""
    mask = torch.rand(tokens.size()) < dropout_rate
    tokens[mask] = pad_idx
    return tokens
...
def main(args):
    ...
    for epoch in range(last_epoch + 1, args.max_epoch):
        ...
        for i, sample in enumerate(progress_bar):
            ...
            # Apply word dropout to source tokens
            sample['src_tokens'] = word_dropout(sample['src_tokens'], args.word_dropout_rate, src_dict.pad_idx)
