...
class BeamSearchNode(object):
    ...
    # add
    def update_step_logps(self, logp):
        # Convert logp to a tensor if it's not already one
        if not isinstance(logp, torch.Tensor):
            logp = torch.tensor(logp)
        self.step_logps.append(logp)

    # add, calculate squared regularization
    def squared_regularizer(self, lambda_val=0.5):
        if len(self.step_logps) == 0:
            # Handle the case where step_logps is empty
            return 0
        squared_sum = lambda_val * torch.sum(torch.pow(torch.stack(self.step_logps), 2))
        return squared_sum
    ...
    # new eval
    def eval(self, alpha=0.0, lambda_val=0.5):
        # Regularized score calculation
        normalizer = (5 + self.length)**alpha / (5 + 1)**alpha
        reg_score = self.logp / normalizer - self.squared_regularizer(lambda_val)
        return reg_score
