...
from seq2seq.beam_regularizer import BeamSearch, BeamSearchNode
    ...
    for i, sample in enumerate(progress_bar):
        ...
        for i in range(batch_size):
            for j in range(args.beam_size):
                ...
                # add here
                node.update_step_logps(log_p)
                # __QUESTION 3: Why do we add the node with a negative score?
                searches[i].add(-node.eval(args.alpha, lambda_val= 0.5), node)
            ...
            for i in range(log_probs.shape[0]):
                for j in range(args.beam_size):
                    ...
                    if next_word[-1] == tgt_dict.eos_idx:
                        ...
                        node.update_step_logps(log_p)
                        search.add_final(-node.eval(args.alpha, lambda_val = 0.5), node)
                    ...
                    # Add the node to current nodes for next iteration
                        ...
                        node.update_step_logps(log_p)
                        search.add(-node.eval(args.alpha, lambda_val = 0.5), node)
