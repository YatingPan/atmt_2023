...
import re
import pickle
import sentencepiece as spm  # add
...
sp = None  # global variable for SentencePiece processor
...
def main(args):
    ...
    global sp #declare sp as global
...
def build_dictionary(filenames, tokenize=bpe_tokenize):
    ...
    for filename in filenames:
        with open(filename, 'r') as file:
            for line in file:
                for symbol in bpe_tokenize(line.strip()):
...
def make_binary_dataset(input_file, output_file, dictionary, tokenize=bpe_tokenize, append_eos=True):
    ...
    with open(input_file, 'r') as inf:
        for line in inf:
            tokens = dictionary.binarize(line.strip(), bpe_tokenize, append_eos, consumer=unk_consumer)
